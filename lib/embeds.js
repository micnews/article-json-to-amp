...
function renderAd (props) {
  const { width, height, network, slot, json, multiSize, multiSizeValidation } = props;
  // This needs to return a string literal because amp-ad needs the json attribute to be wrapped in single-quotes
  return (
    `<amp-ad width="${width}" height="${height}" type="${network}" data-slot="${slot}" json='${JSON.stringify(json)}' data-multi-size="${multiSize}" data-multi-size-validation="${multiSizeValidation}"></amp-ad>`
  );
}
