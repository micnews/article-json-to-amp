...
function renderAd (props) {
  const { label, width, height, network, slot, json, multiSize, multiSizeValidation, layout } = props;
  // This needs to return a string literal because amp-ad needs the json attribute to be wrapped in single-quotes
  return (
    `<h6 class="label--sponsorship">${label || 'Sponsored'}</h6><amp-ad width="${width}" height="${height}" type="${network}" data-slot="${slot}" json='${JSON.stringify(json)}' data-multi-size="${multiSize}" data-multi-size-validation="${multiSizeValidation}" layout="${layout}"></amp-ad>`
  );
}
